...
export function SiteDashboardCard({ site, machineryCount, deploymentPercent }: Props) {
  ...
  const markFinished = () => {
    updateSite.mutate(
      { siteId: site.id, companyId: site.companyId, status: "completed" },
      {
        onSuccess: () => {
          toast({ title: "Site marked complete", description: `${site.name} is now completed.` });
        },
        onError: (err) =>
          toast({
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 flex-col items-end gap-1.5 text-right">
              ...
              <div className="leading-none">
                <div className="font-display text-3xl font-bold tabular-nums text-foreground">{machineryCount}</div>
                <div className="mt-1 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Machinery</div>
              </div>
            </div>
          </div>
          <div className="mt-4">
            <div className="flex justify-between text-xs">
              ...
              <span className="font-medium tabular-nums">{deploymentPercent}%</span>
            </div>
            <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-secondary">
              <div className="h-full bg-gradient-accent" style={{ width: `${deploymentPercent}%` }} />
            </div>
          </div>
          <div className="mt-3 border-t border-border/80 pt-3 text-xs text-muted-foreground">Site utilization overview</div>
        </Link>
      </div>

  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        {(["all", "available", "assigned", "maintenance"] as const).map((k) => (
          <button
            key={k}
            onClick={() => setFilter(k)}
          ...
          >
            {k} <span className="ml-1 opacity-70 tabular-nums">{counts[k]}</span>
          </button>
        ))}
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card shadow-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((m) => {
              const site = sites.find((s) => s.id === m.assignedSiteId);
              return (
                <tr key={m.id} className="border-b border-border last:border-0 hover:bg-secondary/30">
                  ...
                  <td className="px-4 py-2.5 text-muted-foreground">{site?.name ?? <span className="text-muted-foreground/60">—</span>}</td>
          ...
          {editingMachine && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                >
                  {editingMachine.status === "maintenance" ? (
                    <>
                      <option value="maintenance">Maintenance</option>
                      <option value="available">Available</option>
                      <option value="assigned">Assigned</option>
                    </>
                  ) : editingMachine.status === "available" ? (
                    <>
                      <option value="available">Available</option>
                      <option value="assigned">Assigned</option>
                      <option value="maintenance">Maintenance</option>

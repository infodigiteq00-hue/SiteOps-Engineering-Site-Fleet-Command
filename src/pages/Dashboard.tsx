...
const Dashboard = () => {
  ...
  const assigned = machinesInView.filter((m) => m.status === "assigned").length;
  const pending = requests.filter((r) => r.status === "pending").length;
  const utilization = machinesInView.length
    ? Math.round((assigned / Math.max(machinesInView.length, 1)) * 100)
    : 0;
  const totalSitePages = Math.max(1, Math.ceil(sites.length / SITES_PER_PAGE));
  const firstSiteIndex = (sitesPage - 1) * SITES_PER_PAGE;
  const paginatedSites = sites.slice(firstSiteIndex, firstSiteIndex + SITES_PER_PAGE);
  ...
  return (
    <div className="space-y-7">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Active Sites" value={activeSites} icon={Building2} trend={`${sites.length} total`} accent="blue" />
        <StatCard
          label="Machinery Deployed"
          value={`${assigned}/${Math.max(machinesInView.length, 1)}`}
          icon={Wrench}
          trend={`${utilization}% ${user.role === "site_manager" ? "at your sites" : "utilization"}`}
          accent="cyan"
        />
        <StatCard label="Pending Requests" value={pending} icon={ClipboardList} trend="Awaiting approval" accent="orange" />
        <StatCard label="Approved (All time)" value={ledger.length} icon={CheckCircle2} trend="Ledger entries" accent="green" />
      </div>
        ...
        <div>
          <div className="rounded-2xl border border-info/20 bg-gradient-to-b from-info/5 via-card to-card p-4 shadow-card">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-lg font-semibold">Active Sites</h2>
            ...
            <div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-border/80 bg-card/80 px-3 py-2">
              <div className="text-xs text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">{Math.min(firstSiteIndex + SITES_PER_PAGE, sites.length)}</span> of{" "}
                <span className="font-semibold text-foreground">{sites.length}</span> sites
              </div>
